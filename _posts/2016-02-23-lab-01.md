...
为了模拟真实的磁盘，我们使用脚本（讲义提供一份，聪明的你也可以自己写一个^ ^)将bootloader文件填充为512字节。512字节恰好是磁盘一个扇区的大小，因此bootloader文件最终构成了我们生成的磁盘的第一个扇区。在这个扇区中有你写好的引导代码,在BIOS完成检查工作后，便会将执行0x7c00处的地址，开始系统之旅（有心的你可以发现bootloader在完成链接的时候有个选项-Ttext=0x7C00，现在你能想明白这是为什么了吗？）。bootloader起到的作用相当于磁盘的MBR（主引导记录https://zh.wikipedia.org/wiki/%E4%B8%BB%E5%BC%95%E5%AF%BC%E8%AE%B0%E5%BD%95），其中包含了你千辛万苦实现的引导代码。这部分代码完成一系列初始化后通过call跳到了loader的部分，在loader中装载了game的代码。
