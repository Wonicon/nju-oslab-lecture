...
```
##### 解析elf header
利用readsect函数，你可以从内存中载入足够大的数据块，使其完整地包含了elf header。下一步就是对elf header进行解析，这个工作你已经在PA2中做过了，如果对此部分内容有所遗忘，你可以查看PA2讲义中与loader相关的部分和你自己实现的loader的代码。根据解析elf header的结果，你可以从磁盘中将整个游戏加载到内存中。
##### 跳转到游戏
