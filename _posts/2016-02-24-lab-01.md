...
提交格式: 你需要将整个工程打包上传, 特别地, 我们会清除中间结果重新编译, 若编译不通过, 你将损失相应的分数(由于gcc版本带来的问题, 我们会手动处理). 我们会使用脚本进行批量解压缩. 压缩包的命名只能包含你的学号。另外为了防止编码问题, 压缩包中的所有文件都不要包含中文.如果你需要多次提交, 请先手动删除旧的提交记录(提交网站允许下载, 删除自己的提交记录), 否则若脚本解压时出现多次提交相互覆盖的现象, 后果自负.我们只接受以下格式的压缩包: 
tar.gz
tag.bz2
zip
...
分数分布：
实验主体：80%，git的分数奖励是在此基础上计算的，也就是说如果你有均匀合理的git记录，那么你可以不用写实验报告。
实验报告：20%
