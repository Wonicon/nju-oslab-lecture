...
阶段 1 的目标是编写一个 Boot Loader，将上述那个简单的~~根本不是~~游戏装载进内存并执行。
...
现在我们能使用较大的内存了，进入保护模式了，我们可以运行我们的游戏了！话说，我们的游戏在哪呢？

### 加载游戏

BIOS最初只加载 512 字节到内存中，为了使我们的游戏能够运行，我们必须将它加载到内存中来，因为这些剩下的程序还在磁盘中，所以你必须解决磁盘I/O的问题。
...
对独立编址的空间进行 I/O 时需要使用到内联汇编，我们提供了将常见的汇编代码封装成 C 语言的函数的头文件 x86.h （来自jos）。为了理解某个具体的指令的意义，你可能需要阅读i386手册。
...
我们需要一块连续的内存空间来容纳ELF文件头，通过磁盘读取函数将游戏的 elf header 读入到选定的内存位置，通过 phoff 可以找到 ProgramHeader ，通过 phnum 可知道程序段的数目，通过 entry 可以得到程序的入口地址。
