...
为了对实验的流程有明确的认识，能够准确地把握自己的进度，我们将实验划分成了数个小阶段。你们不需要按照阶段提交，只需要一个阶段一个阶段的完成要求即可。

## 阶段 1 Boot Loader 和串口输出

阶段 1 的目标是编写一个 Boot Loader，并且加载你编写的一个简单的 C 程序。

### 系统启动

现在你知道了系统启动时便会执行 0x7c00 处的代码，那么我们能不能在这里就开始运行由 C 代码编译而来的程序呢？答案是否定的，因为我们处于实模式下并且 A20 地址线未打开。

在实模式下，地址线的宽度是 16 位，配合段寄存器只能访问 1M 的内容；并且我们只能执行 16 位的机器指令，而编译器编译 C 代码一般得到的应该是 32 位的机器码（如果你的机器是 64 位的，那么默认生成 64 位代码，注意给 `gcc` 加上 `-m32` 编译选项）。
...
为了将游戏运行起来，首先需要打破这些枷锁。那么我们是先开启保护模式还是先打开A20地址线呢？在开启保护模式前，可以方便地使用 BIOS 中断来开启 A20 地址线，所以我们建议你先打开 A20 地址线。
